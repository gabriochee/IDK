...
$data_populaires = $bdd->prepare("SELECT id_work, primaryTitle, startYear, genre FROM (SELECT work_ratings.id_work, primaryTitle, startYear, genre FROM work_basics JOIN work_ratings ON work_ratings.id_work = work_basics.id_work JOIN work_genres ON work_basics.id_work = work_genres.id_work WHERE startYear = $cette_annee AND work_ratings.averageRating >= 7.0 GROUP BY work_basics.id_work ORDER BY work_ratings.numVotes DESC LIMIT 30) AS subquery ORDER BY RAND() LIMIT 3;");
