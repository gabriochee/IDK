...
<body id="oeuvre">
    <?php 
        ...
        if(isset($_GET['mv'])) {
            ...
            $rep1 = $req1->fetch();
            $req2 = $bdd->query("SELECT region, language FROM work_akas WHERE id_work = {$_GET['mv']} AND ordering ='1'");
            $rep2 = $req2->fetch();
...
<!-- 
            $req1 = $bdd->query("SELECT originalTitle, startYear, runtimeMinutes FROM work_basics WHERE id_work = {$_GET['mv']}");
            $rep1 = $req1->fetch();
        ...
        global $bdd;
        if(isset($_GET['mv'])) {
            $req1 = $bdd->prepare("SELECT originalTitle, startYear, runtimeMinutes FROM work_basics WHERE id_work = ?");
            $req1->execute(array($_GET['mv']));
            $rep1 = $req1->fetch();

            $req2 = $bdd->prepare("SELECT region, language FROM work_akas WHERE id_work = ?");
            $req2->execute(array($_GET['mv']));
            $rep2 = $req2->fetch();

            $req3 = $bdd->prepare("SELECT genre FROM work_genres WHERE id_work = ?");
            $req3->execute(array($_GET['mv']));
            $rep3 = $req3->fetch();
        } else {
            header("location: home.php"); 
            exit(); 
